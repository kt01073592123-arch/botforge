...
// - setChatMenuButton (Mini App tugmasi)
...
const PACK_COMMANDS: Record<string, { command: string; description: string }[]> = {
  ...
  shop_beauty_seoul: [
    { command: "shop", description: "Do'kon (Mini App)" },
    { command: "consultation", description: "AI maslahat" },
    { command: "contact", description: "Aloqa" },
    { command: "operator", description: "Operator" },
  ],
};
...
export async function applyBotPolish(opts: {
  ...
  // setChatMenuButton — bot'ning chat menyusi customer WebApp'ga ko'rsatadi.
  // Bu bot ochganda yuqorida ☰ tugmasi sifatida chiqadi va Mini App'ni ochadi.
  if (bot.tg_username) {
    try {
